# Toolab Standard v1.1 (Normative)

Este documento define el contrato extendido `/_toolab/*` para aplicaciones `toolab-ready`.
La meta es interoperabilidad fuerte entre cualquier adapter y cualquier cliente Toolab-compatible.

## 1. Scope

- Este standard extiende [SPEC_ADAPTER.md](/home/pablo/Projects/Pablo/toolab/docs/SPEC_ADAPTER.md) sin romper compatibilidad.
...
- El prefijo de rutas `/_toolab/` `MUST` mantenerse.
...
- Endpoints `/_toolab/*` `MUST` estar explícitamente habilitados (opt-in).
...
- El payload de error `SHOULD` validar contra `schemas/toolab.standard.error.v1.1.schema.json`.
...
- Respuesta `MUST` validar contra `schemas/toolab.standard.manifest.v1.1.schema.json`.
...
```json
{
  ...
  "links": {
    ...
    "schema_url": "http://localhost:8080/_toolab/schema",
    "profile_url": "http://localhost:8080/_toolab/profile",
    "suggested_flows_url": "http://localhost:8080/_toolab/suggested_flows",
    "invariants_url": "http://localhost:8080/_toolab/invariants",
    "limits_url": "http://localhost:8080/_toolab/limits",
    "environment_url": "http://localhost:8080/_toolab/environment"
  }
}
```
...
- Respuesta `MUST` validar contra `schemas/toolab.standard.profile.v1.1.schema.json`.
...
- Punto único de ingesta para auditoría Toolab.
...
- Respuesta `MUST` validar contra `schemas/toolab.standard.schema.v1.1.schema.json`.
...
```http
GET /_toolab/suggested_flows
```

Contrato:

- Respuesta `MUST` validar contra `schemas/toolab.standard.suggested_flows.v1.1.schema.json`.
...
```http
GET /_toolab/invariants
```

Contrato:

- Respuesta `MUST` validar contra `schemas/toolab.standard.invariants.v1.1.schema.json`.
- `invariants[].id` `MUST` ser único.
- Tipos normativos soportados por Toolab v1:
  - `no_5xx_allowed`
  - `max_4xx_rate`
  - `status_code_rate`
...
- Respuesta `MUST` validar contra `schemas/toolab.standard.limits.v1.1.schema.json`.
...
```http
GET /_toolab/environment
```

Contrato:

- Respuesta `MUST` validar contra `schemas/toolab.standard.environment.v1.1.schema.json`.
...
|---|---|---|
| `manifest` | `GET /_toolab/manifest` | `schemas/toolab.standard.manifest.v1.1.schema.json` |
| `profile` | `GET /_toolab/profile` or `links.profile_url` | `schemas/toolab.standard.profile.v1.1.schema.json` |
| `schema` | `GET /_toolab/schema` | `schemas/toolab.standard.schema.v1.1.schema.json` |
| `openapi` | `links.openapi_url` or `GET /_toolab/openapi` | OpenAPI 3.x (fallback) |
| `suggested_flows` | `GET /_toolab/suggested_flows` | `schemas/toolab.standard.suggested_flows.v1.1.schema.json` |
| `invariants` | `GET /_toolab/invariants` | `schemas/toolab.standard.invariants.v1.1.schema.json` |
| `limits` | `GET /_toolab/limits` | `schemas/toolab.standard.limits.v1.1.schema.json` |
| `environment` | `GET /_toolab/environment` | `schemas/toolab.standard.environment.v1.1.schema.json` |
...
6. Ejecutar workload contra endpoints de negocio (no `/_toolab/*`).
...
Una app es "Toolab Standard v1.1 conformant" si:
...
Este documento define el contrato extendido para maximizar portabilidad y reproducibilidad entre adapters y clientes Toolab.
