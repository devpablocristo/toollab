...
func main() {
	...
	case "gen":
		...
		if len(os.Args) >= 3 && !strings.HasPrefix(os.Args[2], "-") {
			// Legacy style: toolab gen <openapi-spec>
...
func usage() {
	...
	fmt.Println("  toolab generate --from openapi|toolab [flags]")
	fmt.Println("  toolab enrich <scenario.yaml> --from openapi|toolab [flags]")
	fmt.Println("  toolab map --from openapi|toolab [flags]")
	fmt.Println("  toolab explain <run_dir> [flags]")
	fmt.Println("  toolab diff <runA_dir> <runB_dir> [flags]")
	fmt.Println("  toolab gen <openapi-spec> [flags] (alias)")
}

func handleGenerate(args []string) error {
	cmd := flag.NewFlagSet("generate", flag.ContinueOnError)
	from := cmd.String("from", "", "source type: openapi|toolab")
	...
	toolabURL := cmd.String("toolab-url", "", "toolab adapter base URL")
	toolabAuth := cmd.String("toolab-auth", "", "auth for toolab discovery (bearer:ENV or api_key:ENV:header|query:NAME)")
	...
	prefer := cmd.String("prefer", "profile", "toolab discovery preference: profile|endpoints")
	...
	cmd.Var(&requireCaps, "require-capability", "required toolab capability (repeatable)")
...
func handleEnrich(args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: toolab enrich <scenario.yml> [flags]")
	}
	baseScenarioPath := ""
	flagArgs := args
	...
	cmd.Var(&fromValues, "from", "enrichment source: openapi|toolab (repeatable)")
	// backward-compatible booleans
	useOpenAPILegacy := cmd.Bool("from-openapi", false, "enable openapi enrichment")
	useToolabLegacy := cmd.Bool("from-toolab", false, "enable toolab enrichment")

	openapiURL := cmd.String("openapi-url", "", "OpenAPI URL")
	openapiFile := cmd.String("openapi-file", "", "OpenAPI file")
	openapiAuth := cmd.String("openapi-auth", "", "auth for openapi discovery")
	targetBaseURL := cmd.String("target-base-url", "", "target base URL")
	toolabURL := cmd.String("toolab-url", "", "toolab adapter URL")
	toolabAuth := cmd.String("toolab-auth", "", "auth for toolab discovery")
	...
	for _, from := range fromValues {
		...
		case "toolab":
			useToolab = true
		default:
			return fmt.Errorf("invalid --from value %q (expected openapi or toolab)", from)
		}
	}
	if !useOpenAPI && !useToolab {
		return fmt.Errorf("at least one enrichment source is required: --from openapi and/or --from toolab")
	}
...
func handleMap(args []string) error {
	...
	from := cmd.String("from", "", "source: openapi|toolab")
	openapiURL := cmd.String("openapi-url", "", "OpenAPI URL")
	openapiFile := cmd.String("openapi-file", "", "OpenAPI file")
	openapiAuth := cmd.String("openapi-auth", "", "auth for openapi discovery")
	targetBaseURL := cmd.String("target-base-url", "", "target base URL")
...
func handleExplain(args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: toolab explain <run_dir> [flags]")
	}
	runDir := ""
	flagArgs := args
...
func handleDiff(args []string) error {
	if len(args) < 2 {
		return fmt.Errorf("usage: toolab diff <runA_dir> <runB_dir> [flags]")
	}
	runA := ""
	runB := ""
	flagArgs := args
