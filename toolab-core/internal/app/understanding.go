...
import (
	...
	diffmodel "toolab-core/internal/understanding/diff"
...
func BuildSystemMap(ctx context.Context, cfg MapConfig) (*MapResult, error) {
	...
	case "toolab":
		...
		manifest, manifestHash, warn, mErr := toolabFetcher.Manifest(ctx, toolabAuth)
		if mErr != nil {
			return nil, mErr
		}
		warnings = append(warnings, warn...)
		...
		profile, profileHash, profileWarn, pErr := toolabFetcher.Profile(ctx, toolabAuth)
		...
		if pErr == nil {
			...
			systemMap = mapmodel.FromToolab(manifest, profile)
		} else {
			warnings = append(warnings, "profile unavailable, generated partial system map")
			systemMap = mapmodel.FromToolab(manifest, nil)
		}
		if systemMap != nil {
