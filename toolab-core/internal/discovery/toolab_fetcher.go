...
type ToolabFetcher struct {
	BaseURL    string
	HTTPConfig HTTPConfig
	Client     *http.Client
}

func NewToolabFetcher(base string, cfg HTTPConfig) *ToolabFetcher {
	if cfg.TimeoutMS <= 0 {
...
func (f *ToolabFetcher) Manifest(ctx context.Context, auth *AuthConfig) (*Manifest, string, []string, error) {
	var out Manifest
...
func (f *ToolabFetcher) Profile(ctx context.Context, auth *AuthConfig) (*Profile, string, []string, error) {
	var out Profile
...
func (f *ToolabFetcher) OpenAPI(ctx context.Context, auth *AuthConfig) ([]byte, string, []string, error) {
...
func (f *ToolabFetcher) RawJSON(ctx context.Context, suffix string, auth *AuthConfig) (json.RawMessage, string, []string, error) {
	var decoded any
...
func (f *ToolabFetcher) getJSON(ctx context.Context, suffix string, auth *AuthConfig, out any) (string, []string, error) {
	warnings := []string{}
