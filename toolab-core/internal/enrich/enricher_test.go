...
func TestEnrich_AggressiveToolabOnlyOverride(t *testing.T) {
	base := baseScenarioWithTimeout(5000)
	toolabSrc := baseScenarioWithTimeout(1500)
	...
	// Toolab can override defaults in aggressive mode.
	if got := result.Scenario.Workload.Requests[0].TimeoutMS; got != 1500 {
		t.Fatalf("expected toolab override to 1500, got %d", got)
	}
	// OpenAPI must not override toolab/manual values.
