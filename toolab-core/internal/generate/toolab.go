...
func BuildFromToolab(ctx context.Context, fetcher *discovery.ToolabFetcher, openapiFetcher *discovery.OpenAPIFetcher, auth *discovery.AuthConfig, opts ToolabOptions) (*ToolabBuildResult, error) {
	...
	if opts.ToolabURL == "" {
		opts.ToolabURL = strings.TrimRight(opts.TargetBaseURL, "/") + "/_toolab"
	}
	if opts.Prefer == "" {
		opts.Prefer = "profile"
	}
	...
	applyObservabilityFromCapabilities(scn, manifest.Capabilities, opts.ToolabURL, &usedCaps)
...
func defaultScenarioSkeleton(opts ToolabOptions) *scenario.Scenario {
...
func requestsFromEndpoints(ctx context.Context, fetcher *discovery.ToolabFetcher, auth *discovery.AuthConfig, caps []string) ([]scenario.RequestSpec, map[string]string, []string) {
...
func invariantsFromEndpoints(ctx context.Context, fetcher *discovery.ToolabFetcher, auth *discovery.AuthConfig, caps []string, warnings *[]string) []scenario.InvariantConfig {
...
func limitsFromEndpoints(ctx context.Context, fetcher *discovery.ToolabFetcher, auth *discovery.AuthConfig, caps []string, warnings *[]string) map[string]any {
...
func writeTempOpenAPIRaw(raw []byte) (string, error) {
	file, err := os.CreateTemp("", "toolab-openapi-fallback-*.yaml")
	if err != nil {
		return "", err
	}
