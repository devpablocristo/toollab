...
func FromToolab(manifest *discovery.Manifest, profile *discovery.Profile) *SystemMap {
	out := baseMap()
	if manifest != nil {
		out.ServiceIdentity.Name = manifest.AppName
		out.ServiceIdentity.Version = manifest.AppVersion
		out.Resources = append(out.Resources, Resource{Name: "toolab-ready", Kind: "adapter", Tags: manifest.Capabilities})
	}
	if profile == nil {
		out.Partial = true
...
func FromEvidence(bundle *evidence.Bundle) *SystemMap {
	...
	out.ServiceIdentity.Version = bundle.Metadata.ToolabVersion
...
func appendOpenAPIOperation(endpoints *[]Endpoint, path, method string, op *gen.Operation) {
	if op == nil || strings.HasPrefix(path, "/_toolab/") {
		return
	}
