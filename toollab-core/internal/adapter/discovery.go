// Package adapter provides a client for the Toollab Adapter spec v1.
// It probes a target's /_toollab/manifest endpoint and exposes typed
...
type Info struct {
	...
	BaseURL        string   // /_toollab base URL
}
...
// Discover probes targetBaseURL + /_toollab/manifest and returns adapter info.
...
func Discover(ctx context.Context, targetBaseURL string) *Info {
	base := strings.TrimRight(targetBaseURL, "/") + "/_toollab"
...
// Client talks to a toollab adapter over HTTP.
type Client struct {
	baseURL string // e.g. http://localhost:8080/_toollab
	http    *http.Client
}
