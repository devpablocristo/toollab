...
func Enrich(input Inputs) (*Result, error) {
	...
	if input.FromToollab != nil {
		mergeScenario(&out, input.FromToollab, "toollab", input.Strategy, &changes)
	}
	if input.FromOpenAPI != nil {
		// OpenAPI never overwrites existing manual/toollab values in aggressive mode.
