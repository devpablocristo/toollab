...
func BuildFromOpenAPIDoc(doc *gen.OpenAPIDoc, opts OpenAPIOptions) (*scenario.Scenario, []string, error) {
	...
	source := opts.Input
	if source == "" {
		tmp, err := os.CreateTemp("", "toollab-openapi-*.yaml")
		if err != nil {
			return nil, nil, err
		}
	...
	for _, req := range scn.Workload.Requests {
		if strings.HasPrefix(req.Path, "/_toollab/") {
			warnings = append(warnings, "excluded /_toollab endpoint from generated workload")
			continue
		}
