...
func Build(bundle *evidence.Bundle, scn *scenario.Scenario, systemMap *mapmodel.SystemMap, desc *discovery.ServiceDescription) *Report {
	...
	if len(r.AllFlows) > 0 {
		r.MainFlow = findMainFlow(r.AllFlows)
	}
...
func findMainFlow(flows []FlowDetail) FlowDetail {
	if len(flows) == 0 {
		return FlowDetail{}
	}
	main := flows[0]
	for _, f := range flows {
		if f.ErrorRate < main.ErrorRate || (f.ErrorRate == main.ErrorRate && f.AvgLatency < main.AvgLatency) {
			main = f
		}
	}
	return main
