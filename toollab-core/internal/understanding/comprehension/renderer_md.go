...
func RenderMarkdown(r *Report) string {
	...
	// 4. MAIN FLOW
	if r.MainFlow.ID != "" {
		sb.WriteString("## 4. ¿Cuál es el flujo principal?\n\n")
		renderFlow(&sb, r.MainFlow)
	}

	// 5. ALL FLOWS
	sb.WriteString("## 5. ¿Qué se puede hacer con este servicio?\n\n")
	...
	// 6. PAYLOADS
	sb.WriteString("## 6. ¿Qué se envía y qué se recibe?\n\n")
	hasPayloads := false
	for _, f := range r.AllFlows {
	...
	// 7. BEHAVIOR
	sb.WriteString("## 7. ¿Cómo se comporta?\n\n")
	...
	// 8. PERFORMANCE
	sb.WriteString("## 8. ¿Qué tan rápido es?\n\n")
	...
	// 10. CONTRACT
	sb.WriteString("## 10. ¿Está bien hecho?\n\n")
	...
	// 11. VERDICT
	sb.WriteString("## 11. Veredicto y Decisiones\n\n")
	if r.Verdict.ProductionReady {
...
func renderFlow(sb *strings.Builder, f FlowDetail) {
	sb.WriteString(fmt.Sprintf("**%s** — %s\n\n", f.Name, f.Description))
	sb.WriteString(fmt.Sprintf("- Latencia promedio: %dms\n", f.AvgLatency))
	sb.WriteString(fmt.Sprintf("- Tasa de error: %.1f%%\n", f.ErrorRate*100))
	codes := make([]string, len(f.StatusCodes))
	for i, c := range f.StatusCodes {
		codes[i] = fmt.Sprintf("%d", c)
	}
	sb.WriteString(fmt.Sprintf("- Status codes observados: %s\n\n", strings.Join(codes, ", ")))
}
