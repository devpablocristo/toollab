...
export function ScoreRing({ score, grade, size = 120, stroke = 10 }: { score: number; grade: string; size?: number; stroke?: number }) {
  ...
  const c = size / 2
  const gradeColor = grade === 'A' ? '#4ade80' : grade === 'B' ? '#60a5fa' : grade === 'C' ? '#facc15' : grade === 'D' ? '#fb923c' : '#ef4444'
    ...
    <div className="flex flex-wrap gap-x-4 gap-y-1">
      {segments.filter(s => s.value > 0).map((s, i) => (
        <div key={i} className="flex items-center gap-1.5 text-xs">
          ...
          <span className="text-gray-600">({total > 0 ? ((s.value / total) * 100).toFixed(0) : 0}%)</span>
        </div>
      ))}
    </div>
  )
      ...
      {bars.map((b, i) => {
        ...
        return (
          <div key={i} className="flex items-center gap-3">
            <span className="text-xs text-gray-400 w-20 text-right shrink-0">{b.label}</span>
            <div className="flex-1 bg-gray-800/60 rounded-full overflow-hidden" style={{ height }}>
              <div className="h-full rounded-full transition-all duration-500 flex items-center justify-end pr-2"
                style={{ width: `${Math.max(pct, 2)}%`, background: b.color }}>
                {pct > 15 && <span className="text-[10px] font-semibold text-white/90">{b.value}{b.suffix ?? ''}</span>}
              </div>
            </div>
            {pct <= 15 && <span className="text-xs text-gray-400 shrink-0">{b.value}{b.suffix ?? ''}</span>}
          </div>
        )
      })}
    </div>
...
export function StackedBar({ segments, height = 28 }: { segments: StackSegment[]; height?: number }) {
  const total = segments.reduce((s, seg) => s + seg.value, 0)
  if (total === 0) return <div className="bg-gray-800 rounded-full" style={{ height }} />
  return (
    <div className="flex rounded-full overflow-hidden" style={{ height }}>
      {segments.filter(s => s.value > 0).map((s, i) => (
        <div key={i} className="flex items-center justify-center transition-all duration-500"
          ...
          {(s.value / total) > 0.08 && (
            <span className="text-[10px] font-bold text-white/90">{s.value}</span>
          )}
        </div>
      ))}
    </div>
  ...
  return (
    <svg width={totalW} height={height} className="shrink-0">
      {values.map((v, i) => {
        ...
        return <rect key={i} x={i * (barW + gap)} y={height - h} width={barW} height={h} rx={1.5} fill={colors[i % colors.length]} />
      })}
    </svg>
  )
}
...
export function PercentRing({ value, size = 56, stroke = 5, color = '#60a5fa' }: { value: number; size?: number; stroke?: number; color?: string }) {
  const r = (size - stroke) / 2
  const circ = 2 * Math.PI * r
  const c = size / 2
      ...
      {cells.map((cell, i) => (
        <div key={i} className="flex flex-col items-center gap-0.5" title={`${cell.label}: ${cell.value}`}>
          <div className="w-7 h-7 rounded flex items-center justify-center text-[10px] font-bold"
            style={{ background: cell.color + '30', color: cell.color }}>
            {cell.value}
          </div>
          <span className="text-[9px] text-gray-600">{cell.label}</span>
        </div>
      ))}
    </div>
  )
