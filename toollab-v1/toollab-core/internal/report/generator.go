...
if [[ -n "${TOOLLAB_CMD:-}" ]]; then
  read -r -a TOOLLAB_ARR <<<"${TOOLLAB_CMD}"
elif command -v toollab >/dev/null 2>&1; then
  TOOLLAB_ARR=("toollab")
elif command -v go >/dev/null 2>&1 && [[ -f "$REPO_ROOT/toollab-core/cmd/toollab/main.go" ]]; then
  TOOLLAB_ARR=("go" "-C" "$REPO_ROOT/toollab-core" "run" "./cmd/toollab")
else
  echo "toollab command not found. Set TOOLLAB_CMD or install toollab in PATH." >&2
  exit 127
fi

RUN_OUTPUT="$("${TOOLLAB_ARR[@]}" run "$SCENARIO_PATH" --out "$OUT_BASE")"
echo "$RUN_OUTPUT"
